...
def test_find_matching_images_without_missing_match() -> None:
...
def test_find_matching_images_with_missing_match() -> None:
    ...
    correct = correct.set_index(["Subject ID", "SCANDATE"])
    with pytest.warns(UserWarning, match="Could not find matching images for:"):
        matches = adnipy.get_matching_images(left, right)
    pd.testing.assert_frame_equal(correct, matches)
